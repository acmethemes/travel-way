 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'Travel_Way_Gallery' ) ) {

    class Travel_Way_Gallery extends WP_Widget {
        ...
        function __construct() {
            parent::__construct(
                    ...
                    esc_html__('AT Gallery Section', 'travel-way'),
                    /*Widget description*/
                    array(
                            'description' => esc_html__( 'Show Gallery Section.', 'travel-way' )
                    )
            );
        }
        ...
        public function form( $instance ) {
            ...
            <br/>
            <small><?php esc_html_e( 'Add Page, Reorder and Remove. Please do not forget to add Featured Image on selected pages. ', 'travel-way' ); ?></small>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'image_popup_type' ) ); ?>"><?php esc_html_e( 'Image Popup Type', 'travel-way' ); ?></label>
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'image_popup_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'image_popup_type' ) ); ?>" >
                    <?php
                    $travel_way_gallery_image_popup = travel_way_gallery_image_popup();
                    foreach ( $travel_way_gallery_image_popup as $key => $value ){
                        ?>
            ...
            <p>
                ...
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'travel_way_img_size' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'travel_way_img_size' ) ); ?>">
			        ...
			        foreach( $travel_way_image_sizes as $key => $travel_way_column_array ){
				        echo ' <option value="'.esc_attr( $key ).'" '.selected( esc_attr( $travel_way_img_size ), $key, 0). '>'.esc_html( $travel_way_column_array ).'</option>';
			        }
			        ?>
                </select>
            </p>
        ...
        public function update( $new_instance, $old_instance ) {
	        ...
	        $travel_way_image_sizes              = travel_way_get_image_sizes_options();
	        $instance[ 'travel_way_img_size' ]   = travel_way_sanitize_choice_options( $new_instance[ 'travel_way_img_size' ], $travel_way_image_sizes, 'large' );

	        $travel_way_gallery_image_popup      = travel_way_gallery_image_popup();
	        $instance[ 'image_popup_type' ]         = travel_way_sanitize_choice_options( $new_instance[ 'image_popup_type' ], $travel_way_gallery_image_popup, 'gallery' );
            ...
            <section id="<?php echo esc_attr( $unique_id ); ?>" class="at-widgets acme-gallery">
                <div class="full-width-container">
                    ...
                    if( !empty ( $post_in ) && is_array( $post_in ) ) :
	                    ...
	                    $gallery_query = new WP_Query( $post_in_page_args );
                        ?>
                        <div class="row fullwidth-row">
                            ...
                            if ( $gallery_query->have_posts() ):
                                ...
                                while( $gallery_query->have_posts() ):$gallery_query->the_post();
	                                if( 1 == $column_number ){
		                                $travel_way_column = " col-xs-12";
	                                }
                                    elseif( 2 == $column_number ){
		                                $travel_way_column = " col-sm-6";
	                                }
                                    elseif( 3 == $column_number ){
		                                $travel_way_column = " col-sm-4 col-md-4";
	                                }
                                    elseif( 4 == $column_number ){
		                                $travel_way_column = ' col-sm-3 col-md-3';
	                                }
	                                else{
		                                $travel_way_column = " col-sm-3 col-md-3";
	                                }
                                    ?>
                                    <div class="at-gallery-item <?php echo esc_attr( $travel_way_column ); ?>">
                                        <div class="gallery-inner-item <?php echo esc_attr( $animation );?>">
                                            ...
                                            if( has_post_thumbnail() ):
                                                $image_url = wp_get_attachment_image_src( get_post_thumbnail_id(), $travel_way_img_size );
                                            else:
    ...
    } // Class Travel_Way_Gallery ends here
}