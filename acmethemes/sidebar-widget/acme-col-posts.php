 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'Travel_Way_Posts_Col' ) ) {
    /**
     * Class for adding widget
     *
     * @package Acme Themes
     * @subpackage Travel_Way_Posts_Col
     * @since 1.0.0
     */
    class Travel_Way_Posts_Col extends WP_Widget {
        ...
        private $defaults = array(
	        ...
	        'travel_way_post_cat'           => -1,
	        'travel_way_post_tag'           => -1,
            'post_number'                   => 4,
        ...
        function __construct() {
            parent::__construct(
                    ...
                    esc_html__('AT Posts Column', 'travel-way'),
                    /*Widget description*/
                    array(
                            'description' => esc_html__( 'Show posts from selected category with advanced options', 'travel-way' )
                    )
            );
        }
        ...
        public function form( $instance ) {
	        ...
	        $title                      = esc_attr( $instance['travel_way_widget_title'] );
	        $post_advanced_option       = esc_attr( $instance['post_advanced_option'] );
	        $travel_way_post_cat       = esc_attr( $instance['travel_way_post_cat'] );
	        $travel_way_post_tag       = esc_attr( $instance['travel_way_post_tag'] );
	        ...
	        ?>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'unique_id' ) ); ?>"><?php _e( 'Section ID', 'travel-way' ); ?>:</label>
                ...
                <br />
                <small><?php _e('Enter a Unique Section ID. You can use this ID in Menu item for enabling One Page Menu.','travel-way')?></small>
            </p>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'travel_way_widget_title' ) ); ?>">
                    <?php esc_html_e( 'Title', 'travel-way' ); ?>
                </label>
                <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'travel_way_widget_title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'travel_way_widget_title' ) ); ?>" type="text" value="<?php echo $title; ?>" />
            </p>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'post_advanced_option' ) ); ?>"><?php esc_html_e( 'Show', 'travel-way' ); ?></label>
                <select class="widefat at-post-advanced-option" id="<?php echo esc_attr( $this->get_field_id( 'post_advanced_option' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'post_advanced_option' ) ); ?>" >
			        <?php
			        $post_advanced_options = travel_way_post_advanced_options();
            ...
            <p class="post-cat post-select">
                <label for="<?php echo esc_attr( $this->get_field_id('travel_way_post_cat') ); ?>">
                ...
                $travel_way_dropown_cat = array(
                    ...
                    'hide_empty'         => 1,
                    'echo'               => 1,
                    'selected'           => $travel_way_post_cat,
                    'hierarchical'       => 1,
                    'name'               => $this->get_field_name('travel_way_post_cat'),
                    'id'                 => $this->get_field_id('travel_way_post_cat'),
                    'class'              => 'widefat',
                    'taxonomy'           => 'category',
                    'hide_if_empty'      => false,
                );
            ...
            <p class="post-tag post-select">
                <label for="<?php echo esc_attr( $this->get_field_id('travel_way_post_tag') ); ?>">
		        ...
		        $travel_way_dropown_cat = array(
			        ...
			        'hide_empty'         => 1,
			        'echo'               => 1,
			        'selected'           => $travel_way_post_tag,
			        'hierarchical'       => 1,
			        'name'               => $this->get_field_name('travel_way_post_tag'),
			        'id'                 => $this->get_field_name('travel_way_post_tag'),
			        'class'              => 'widefat',
			        'taxonomy'           => 'post_tag',
			        'hide_if_empty'      => false,
		        );
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'content_from' ) ); ?>"><?php _e( 'Content From', 'travel-way' ); ?>:</label>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'content_words' ) ); ?>">
			        <?php esc_html_e( 'Number of words on content', 'travel-way' ); ?>
                </label>
                <br/>
                <small>
            ...
            <p>
                ...
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'orderby' ) ); ?>" >
			        <?php
			        $travel_way_post_orderby = travel_way_post_orderby();
			        foreach ( $travel_way_post_orderby as $key => $value ){
				        ?>
            ...
            <p>
                ...
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'travel_way_img_size' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'travel_way_img_size' ) ); ?>">
			        <?php
			        $travel_way_image_sizes = travel_way_get_image_sizes_options();
			        foreach( $travel_way_image_sizes as $key => $travel_way_column_array ){
				        echo ' <option value="'.esc_attr( $key ).'" '.selected( esc_attr( $travel_way_img_size ), $key, 0). '>'.esc_attr( $travel_way_column_array ).'</option>';
			        }
			        ?>
                </select>
            </p>
        ...
        public function update( $new_instance, $old_instance ) {
	        ...
	        $post_advanced_options                  = travel_way_post_advanced_options();
	        $instance['post_advanced_option']       = travel_way_sanitize_choice_options( $new_instance['post_advanced_option'], $post_advanced_options, 'recent' );
	        
	        $instance['travel_way_post_cat']     = ( isset( $new_instance['travel_way_post_cat'] ) ) ? esc_attr( $new_instance['travel_way_post_cat'] ) : '';
	        $instance['travel_way_post_tag']     = ( isset( $new_instance['travel_way_post_tag'] ) ) ? esc_attr( $new_instance['travel_way_post_tag'] ) : '';
	        ...
	        $travel_way_post_orderby             = travel_way_post_orderby();
	        $instance['orderby']                    = travel_way_sanitize_choice_options( $new_instance['orderby'], $travel_way_post_orderby, 'date' );

	        $travel_way_post_order               = travel_way_post_order();
	        $instance['order']                      = travel_way_sanitize_choice_options( $new_instance['order'], $travel_way_post_order, 'DESC' );

	        $travel_way_image_sizes             = travel_way_get_image_sizes_options();
	        $instance['travel_way_img_size']    = travel_way_sanitize_choice_options( $new_instance['travel_way_img_size'], $travel_way_image_sizes, 'post-thumbnail' );
        ...
        public function widget($args, $instance) {
	        ...
	        $travel_way_post_cat         = esc_attr( $instance['travel_way_post_cat'] );
	        $travel_way_post_tag         = esc_attr( $instance['travel_way_post_tag'] );
	        $title                          = !empty( $instance['travel_way_widget_title'] ) ? esc_attr( $instance['travel_way_widget_title'] ) : get_cat_name($travel_way_post_cat);
	        ...
	        switch ( $post_advanced_option ) {
		        ...
		        case 'cat' :
			        $query_args['cat'] = $travel_way_post_cat;
			        break;

		        case 'tag' :
			        $query_args['tag'] = $travel_way_post_tag;
			        break;
	        }
                ...
                <section id="<?php echo esc_attr( $unique_id ); ?>" class="at-widgets acme-col-posts <?php echo $bg_gray_class;?>">
                    <div class="container">
                        ...
                        if ( ! empty( $title ) ) {
	                        ...
	                        if( -1 != $travel_way_post_cat ){
		                        echo "<div class='at-cat-color-wrap-".$travel_way_post_cat."'>";
	                        }
                        ...
                        <div <?php echo esc_attr( $div_attr ); ?>>
                            ...
                            $thumb = $travel_way_img_size;
                            $travel_way_list_classes = 'single-list';
                            $travel_way_words = $content_words;
                                    ...
                                    <article id="post-<?php the_ID(); ?>" <?php post_class( $animation ); ?>>
                                        <div class="content-wrapper">
                                            ...
                                            <div class="entry-content <?php echo esc_attr( $no_blog_image ); ?>">
                                                ...
                                                <?php
                                                if( 0 != $travel_way_words ){
	                                                ?>
                                                    <div class="details">
		                                                <?php
		                                                travel_way_advanced_content( $travel_way_words, $content_from );
		                                                ?>
                                                    </div>
	                                                <?php
                                                }
                                                ?>
                                                <?php
                                                if( !empty( $travel_way_read_more_text ) ){
	                                                echo '<a href="'.esc_url(get_permalink()).'" class="btn btn-primary">';
	                                                echo esc_html( $travel_way_read_more_text );
	                                                echo '</a>';
                                                } 
                                                ?>
                                            </div><!-- .entry-content -->
    ...
    } // Class Travel_Way_Posts_Col ends here
}